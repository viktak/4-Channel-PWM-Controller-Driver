...
#define DEFAULT_HOSTNAME "led-pwm"
...
#define DEFAULT_MQTT_TOPIC "led-pwm"
...
void settings::PrintSettings()
{
    ...
    SerialMon.printf("App name\t\t%s\r\nAdmin password\t\t%s\r\nAP SSID\t\t\t%s\r\nAP Password\t\t%s\r\nTimezone\t\t%i\r\nHostname:\t\t%s\r\n"
                     ...
                     mqttServer, mqttPort, mqttTopic, mqttUserName, mqttPassword, mqttRetain == true ? "true" : "false", ssid,
                     heartbeatInterval, flipButtons == true ? "true" : "false",
...
void settings::Save()
{
    ...
    prefs.putBool("MQTT_RETAIN", mqttRetain);

    //  Operation
    prefs.putUChar("OP_MODE", opMode);
    prefs.putBool("FLIP_BUTT", flipButtons);
...
void settings::Load(bool LoadDefaults)
{
    ...
    mqttRetain = prefs.getBool("MQTT_RETAIN", false);

    //  Operation
    opMode = static_cast<OPERATION_MODES>(prefs.getUChar("OP_MODE", DEFAULT_OPERATION_MODE));
    flipButtons = prefs.getBool("FLIP_BUTT", false);
